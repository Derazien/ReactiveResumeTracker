...
export class TagExtractionService {
  ...
  async extractJobTags(jobDescription: string): Promise<TagExtractionResult> {
    ...
    try {
      ...
      this.logger.log(`Extracted ${finalTags.length} tags in ${processingTime}ms using NLP libraries`);
  ...
  private async extractTechnicalSkills(text: string): Promise<string[]> {
    ...
    const programmingLanguages = [
      ...
      'kotlin', 'scala', 'r', 'matlab', 'perl', 'shell', 'bash', 'powershell', 'sql', 'html', 'css'
    ];

    // Frameworks and libraries
    const frameworks = [
      'react', 'angular', 'vue', 'node.js', 'express', 'django', 'flask', 'spring', 'laravel',
      'rails', 'asp.net', '.net', 'jquery', 'bootstrap', 'tailwind', 'sass', 'less'
    ];
    ...
    const cloudDevOps = [
      ...
      'ansible', 'chef', 'puppet', 'vagrant', 'nginx', 'apache', 'linux', 'unix', 'windows'
    ];

    // Tools and technologies
    const tools = [
      'git', 'jira', 'confluence', 'slack', 'figma', 'adobe', 'photoshop', 'illustrator',
      'sketch', 'invision', 'zeplin', 'postman', 'swagger', 'api', 'rest', 'graphql', 'websocket'
    ];

    const allTechTerms = [...programmingLanguages, ...frameworks, ...databases, ...cloudDevOps, ...tools];
  ...
  private async extractSoftSkills(text: string): Promise<string[]> {
    const softSkillPatterns = [
      ...
      'analytical', 'creativity', 'innovation', 'adaptability', 'flexibility', 'time-management',
      'organization', 'attention-to-detail', 'multitasking', 'decision-making', 'negotiation',
      'presentation', 'public-speaking', 'mentoring', 'coaching', 'project-management',
  ...
  private async extractIndustryTerms(text: string): Promise<string[]> {
    ...
    // Industry buzzwords
    const industryTerms = [
      'fintech', 'healthtech', 'edtech', 'proptech', 'insurtech', 'regtech', 'martech', 'adtech',
      'e-commerce', 'saas', 'paas', 'iaas', 'b2b', 'b2c', 'b2g', 'enterprise', 'startup',
      'digital-transformation', 'automation', 'ai', 'machine-learning', 'artificial-intelligence',
      'blockchain', 'cryptocurrency', 'iot', 'internet-of-things', 'cybersecurity', 'data-science',
      'big-data', 'analytics', 'business-intelligence', 'data-visualization', 'reporting'
    ];
   ...
   * Score and deduplicate tags
   */
  private scoreAndDeduplicateTags(tagItems: Array<{ tag: string; weight: number; type: string }>) {
    const tagScores: { [key: string]: { score: number; types: Set<string> } } = {};

    // Aggregate scores for duplicate tags
    tagItems.forEach(({ tag, weight, type }) => {
      const normalizedTag = tag.toLowerCase().trim();
      if (normalizedTag.length < 2) return;
    ...
    Object.entries(tagScores).forEach(([tag, data]) => {
