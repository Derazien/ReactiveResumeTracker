...
import { Database, PencilSimple, Plus, ArrowCounterClockwise } from '@phosphor-icons/react';
import { Button, Tooltip } from '@reactive-resume/ui';
import { cn } from '@reactive-resume/utils';
import { useState } from 'react';
import { AnimatePresence } from 'framer-motion';

interface ContentRelationshipIndicatorProps {
...
export const useContentRelationship = (contentId: string | null, sourceContentId: string | null) => {
  const getRelationshipType = () => {
    if (contentId && !sourceContentId) return 'added-from-library';
    if (contentId && sourceContentId) return 'variant-from-library';
    if (!contentId && sourceContentId) return 'modified';
